...
class TemporalDataset(object):
    def __init__(self, name: str, use_continuous_time: bool = False):
        ...
        # Load continuous time mappings if requested
        self.ts_normalized = None
        if use_continuous_time:
            try:
                ts_norm_path = self.root / 'ts_normalized.pickle'
                if ts_norm_path.exists():
                    with open(ts_norm_path, 'rb') as f:
                        self.ts_normalized = pickle.load(f)
                    print(f"Loaded continuous time mappings for {name}")
                else:
                    print(f"Warning: Continuous time requested but {ts_norm_path} not found")
                    print("Please run preprocess_continuous_time.py first")
            except Exception as e:
                print(f"Error loading continuous time mappings: {e}")
    ...
    def get_continuous_time(self, timestamp_id):
        ...
        if isinstance(timestamp_id, torch.Tensor):
            # Handle batch of timestamp IDs
            return torch.tensor([self.ts_normalized[int(tid)] for tid in timestamp_id.cpu().numpy()],
                              dtype=torch.float32)
        else:
            return self.ts_normalized[timestamp_id]
    ...
    def eval(
        ...
        # Store original timestamp IDs before conversion
        original_timestamp_ids = None
        if self.use_continuous_time and self.ts_normalized is not None:
            original_timestamp_ids = examples[:, 3].clone()  # Keep original IDs
            examples_continuous = examples.clone().float()
            timestamp_ids = examples[:, 3].cpu().numpy()
            continuous_times = torch.tensor(
                [self.ts_normalized[int(tid)] for tid in timestamp_ids],
                dtype=torch.float32
            ).cuda()
            examples_continuous[:, 3] = continuous_times
            examples = examples_continuous
        ...
        for m in missing:
            q = examples.clone()
            ts_ids = original_timestamp_ids.clone() if original_timestamp_ids is not None else None
            if n_queries > 0:
                permutation = torch.randperm(len(examples))[:n_queries]
                q = examples[permutation]
                if ts_ids is not None:
                    ts_ids = ts_ids[permutation]
            if m == 'lhs':
                tmp = torch.clone(q[:, 0])
                q[:, 0] = q[:, 2]
                q[:, 2] = tmp
                q[:, 1] += self.n_predicates // 2
            ranks = model.get_ranking(q, self.to_skip[m], batch_size=500, timestamp_ids=ts_ids)
